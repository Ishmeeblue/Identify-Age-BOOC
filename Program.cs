...
namespace Identify_Age_BOOC
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Category(age, ref stage,ref generation);
            CategoryStage(age,ref stage, ref generation);
            Console.WriteLine($"{age} years old, {generation}, {stage}");
        }
        public static void Category(int age,ref string stage, ref string generation)
        {
            if (age >= 79 && age <= 96)
            {
        ...
        public static void CategoryStage(int age, ref string stage,ref string generation)
        {
            if (age >= 0 && age <= 3)
            {
                stage = "Infant";
